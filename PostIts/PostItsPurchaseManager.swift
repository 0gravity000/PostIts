...
class PostItsPurchaseManager: NSObject,SKPaymentTransactionObserver {
    ...
    /// シングルトン
    class func sharedManager() -> PostItsPurchaseManager{
...
@objc protocol PostItsPurchaseManagerDelegate {
    //課金完了
    optional func purchaseManager(purchaseManager: PostItsPurchaseManager!, didFinishPurchaseWithTransaction transaction: SKPaymentTransaction!, decisionHandler: ((complete : Bool) -> Void)!)
    //課金完了(中断していたもの)
    optional func purchaseManager(purchaseManager: PostItsPurchaseManager!, didFinishUntreatedPurchaseWithTransaction transaction: SKPaymentTransaction!, decisionHandler: ((complete : Bool) -> Void)!)
    //リストア完了
    optional func purchaseManagerDidFinishRestore(purchaseManager: PostItsPurchaseManager!)
    //課金失敗
    optional func purchaseManager(purchaseManager: PostItsPurchaseManager!, didFailWithError error: NSError!)
    //承認待ち(ファミリー共有)
    optional func purchaseManagerDidDeferred(purchaseManager: PostItsPurchaseManager!)
}
